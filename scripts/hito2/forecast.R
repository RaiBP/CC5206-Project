...
x2 <- na.kalman(tsPSO2)
x1 <- ts(tsACVP$amount,start=c(2001,1), end=c(2017,12),frequency=12) ##aqui se puede cambiar el TS de egreso por el que se desee
...
endTrain <- ymd("2017-01-01") ## fecha de término para train data e egresos
...
## modelo
fit1 <- auto.arima(train1, xreg = train2) ### D=1 activa la estacionalidad
...
write.csv(as.data.frame(accuracy(fcast2,test)),file="plots\\hito2\\PuchNeumoniaConEstacSinContaminantes.csv")
...
####### SCATTERPLOTS ########
library(astsa)
lag2.plot (x3,x1,12) ##(variable 1, variable2, número de lags a plotear)